...
class writer:
    ...
    def groundedTarski( self, planner, problem ) :
        ...
        for action in problem.actions.values():
            ...
            if not isinstance(action.precondition, CompoundFormula):
                formulas = [action.precondition]
            else:
                formulas = action.precondition.subformulas
            action_tarski = self.getGoal(formulas)
