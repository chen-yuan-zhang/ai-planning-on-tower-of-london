...
def main():
    problem_id = 0
    while os.path.exists("TOL_" + str(problem_id)):
        # use new PDDL model (with handempty predicate)

        folder = "TOL_" + str(problem_id)
        print(folder)
        domain = folder + "/domain.pddl"
        instance = folder + "/problem.pddl"

        with open(folder + "/info_v.json") as f:
        ...
        #
        # """ASTAR"""
        # astar= AStarSearch(heursitic_function=partial(gc_heuristic, planner=planner, lw=lw), repeat_check=False)
        ...
        #         succ_dict[str(info[-1].state)] = succ_dict.get(str(info[-1].state), 0) + 1
        #
        # info_dict['action_count(astar+gc)'] = succ_dict
        ...
        info_dict['action_count'] = action_count
        info_dict['num_expanded_nodes(tree+gc)'] = result['time']

        with open(folder + "/info_v_new.json", 'w') as f:
